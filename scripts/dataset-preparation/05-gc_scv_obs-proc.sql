...
BEGIN
  ...
  -- ============================================================
  -- Step 1: Materialize gc_scv_obs — the 10-table join runs once
  --         obs_content is carried here for UDF parsing in steps 2-3
  -- ============================================================
  SET sql = """
    CREATE TEMP TABLE gc_scv_obs
    CLUSTER BY scv_obs_id AS
    SELECT
      ...
      def.rank as vcvc_rank
    FROM `variation_tracker.report_submitter` rs
    JOIN `@schema.scv_summary` scv
    ON
      rs.submitter_id = scv.submitter_id
      AND
      rs.type = 'GC'
    CROSS JOIN UNNEST(scv.clinical_assertion_observation_ids) as cao_id
    JOIN `@schema.clinical_assertion_observation` cao
    ON
      cao.id = cao_id
    LEFT JOIN `@schema.clinical_assertion_trait_set` caots
    ON
      caots.id = cao.clinical_assertion_trait_set_id
    LEFT JOIN `@schema.single_gene_variation` sgv
    ON
      sgv.variation_id = scv.variation_id
    LEFT JOIN `@schema.gene` g
    ON
      sgv.gene_id = g.id
    JOIN `@schema.variation` v
    ON
      v.id = scv.variation_id
    JOIN `@schema.variation_archive` vcv
    ON
      vcv.variation_id = scv.variation_id
    JOIN `@schema.variation_archive_classification` vac
    ON
      vac.vcv_id = vcv.id
      AND
    ...
    ON
      LOWER(vac.review_status) = def.review_status
      AND
  ...
  -- ============================================================
  -- Step 2: Parse methods from obs_content (parseMethods UDF, once)
  -- ============================================================
  EXECUTE IMMEDIATE """
    CREATE TEMP TABLE gc_scv_obs_testing
    CLUSTER BY scv_obs_id AS
    SELECT
      gso.scv_obs_id,
  ...
  -- ============================================================
  -- Step 3: Parse observed data from obs_content (parseObservedData UDF, once)
  -- ============================================================
  EXECUTE IMMEDIATE """
    CREATE TEMP TABLE gc_scv_obs_attribs
    CLUSTER BY scv_obs_id AS
    SELECT
      gso.scv_obs_id,
      MAX(
  ...
  -- ============================================================
  -- Step 4: Clinical features (parseXRefs UDF on trait content)
  -- ============================================================
  SET sql = """
    CREATE TEMP TABLE gc_clinical_feature_set
    CLUSTER BY scv_obs_id AS
    WITH clinical_feature AS (
      SELECT
        gso.scv_obs_id,
        xref.id as xref_id,
      ...
      JOIN `@schema.clinical_assertion_trait` obs_trait
      ON
  ...
  -- ============================================================
  -- Step 5: Derived tables (no UDF calls, lightweight)
  -- ============================================================

  -- 5a: Case counts
  EXECUTE IMMEDIATE """
    CREATE TEMP TABLE gc_scv_obs_case_count AS
    SELECT
      gso.id,
  ...
  -- 5b: Patient identification
  EXECUTE IMMEDIATE """
    CREATE TEMP TABLE gc_scv_obs_patient
    CLUSTER BY scv_obs_id AS
    WITH gc_scv_obs_patient_prep AS (
      SELECT DISTINCT
        gso.scv_obs_id,
        gsoa.sample_id,
        gso.local_key,
  ...
  -- 5c: Co-occurring variants in same gene per patient
  EXECUTE IMMEDIATE """
    CREATE TEMP TABLE obs_patient_co_occuring_scv
    CLUSTER BY scv_obs_id AS
    WITH patient_co_occurring_same_gene AS (
      SELECT
        patient_id,
        gso.gene_id,
  ...
  -- ============================================================
  -- Step 6: Final assembly — all temp tables, no UDF calls
  -- ============================================================
  SET sql = """
    CREATE OR REPLACE TABLE `@schema.gc_scv_obs` AS
    SELECT
      gso.scv_obs_id,
      gso.variation_id,
      gso.id,
  ...
  -- cleanup temp tables
  DROP TABLE IF EXISTS gc_scv_obs;
  DROP TABLE IF EXISTS gc_scv_obs_testing;
  DROP TABLE IF EXISTS gc_scv_obs_attribs;
  DROP TABLE IF EXISTS gc_clinical_feature_set;
  DROP TABLE IF EXISTS gc_scv_obs_case_count;
  DROP TABLE IF EXISTS gc_scv_obs_patient;
  DROP TABLE IF EXISTS obs_patient_co_occuring_scv;
