...
BEGIN
  DECLARE rcv_accession_classification_table_exists BOOL;
  DECLARE variation_archive_classification_table_exists BOOL;
  DECLARE rcv_mapping_table_exists BOOL;
  DECLARE statement_type_column_exists BOOL;
  DECLARE agg_classification_column_exists BOOL;
  ...
  IF (project_id <> 'clingen-stage') THEN
    ...
    CALL `clinvar_ingest.check_column_exists`(schema_name, 'clinical_assertion', 'statement_type', statement_type_column_exists);
    ...
    CALL `clinvar_ingest.check_table_exists`(schema_name, 'rcv_accession_classification', rcv_accession_classification_table_exists);
    ...
    ELSE
      ...
      CALL `clinvar_ingest.check_column_exists`(schema_name, 'rcv_accession_classification', 'agg_classification', agg_classification_column_exists);
    ...
    CALL `clinvar_ingest.check_table_exists`(schema_name, 'variation_archive_classification', variation_archive_classification_table_exists);
    ...
    -- TABLE 4. Apply RCV -2- SCV mappings fkeys
    -- check that the rcv_mapping table exists as an indicator that the rcv_accession_id values need to be set on the clinical_assertion table.
